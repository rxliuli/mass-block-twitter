import { router, useRoute } from '$lib/route.svelte'
...
describe('useRoute', () => {
  ...
  it('async', () => {
    ...
    router.path = '/home'
    router.routes = [
      { path: '/home', component: () => import('./components/HomeView.svelte') },
      { path: '/about', component: () => import('./components/AboutView.svelte') },
    ]
    expect(route.path).toBe('/home')
    expect(route.matched).toBe(router.routes[0])
    router.path = '/about'
